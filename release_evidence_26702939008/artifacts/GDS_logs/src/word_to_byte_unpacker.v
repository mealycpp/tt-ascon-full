/*
 * word_to_byte_unpacker.v — split 64-bit words into UART bytes.
 *
 * Area/fanout surgery:
 *   - Only control registers are reset.
 *   - Data buffer/output byte are not reset; valid flag controls use.
 */
...
module word_to_byte_unpacker (
    ...
    assign in_word_ready = (bytes_left == 4'd0) && !out_byte_valid;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if ((bytes_left != 4'd0) && (!out_byte_valid || out_byte_ready)) begin
                case (emit_idx)
