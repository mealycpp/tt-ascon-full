...
async def send_xof_cmd(dut, vec):
    msg_len = len(vec["msg"])
    cs_len = len(vec["cs"])
    out_len = vec["out_len"]
...
@cocotb.test()
async def test_xof_cxof_one_generated_top_uart_vector(dut):
    """Top-level UART XOF/CXOF: one generated Verilog KAT vector per simulation."""

    cocotb.start_soon(Clock(dut.clk, CLK_PERIOD_NS, unit="ns").start())

    tb_rel = os.environ.get(
        "XOF_TB",
        "test/sdmc_top_uart_xof_cxof_kat_massive/tb_xof_c0001_m0_cs0_out64.v",
    )
    tb_path = REPO_ROOT / tb_rel
    vec = parse_xof_tb(tb_path)
    ...
    await send_xof_cmd(dut, vec)

    # Generated TB sends customization string first, then message.
    for b in vec["cs"]:
        await uart_send_byte(dut, b)
    for b in vec["msg"]:
        await uart_send_byte(dut, b)
    ...
    if len(rx_bytes) < vec["out_len"]:
        raise TimeoutError(
            f"{vec['mode_name']} timeout name={vec['name']} "
            f"got_bytes={len(rx_bytes)} expected={vec['out_len']}"
        )
    ...
    dut._log.info("VECTOR XOF_TB=%s", tb_rel)
    dut._log.info(
        "INPUT_HEX mode=%s name=%s msg_bytes=%d cs_bytes=%d msg=%s cs=%s",
        vec["mode_name"], vec["name"], len(vec["msg"]), len(vec["cs"]),
        vec["msg"].hex(), vec["cs"].hex()
    )
    dut._log.info("GOT_%s name=%s got=%s", vec["mode_name"], vec["name"], got.hex())
    dut._log.info("EXP_%s name=%s exp=%s", vec["mode_name"], vec["name"], expected.hex())

    assert got == expected, (
        f"{vec['mode_name']} mismatch name={vec['name']} "
        f"got={got.hex()} exp={expected.hex()}"
    )
    ...
    dut._log.info(
        "PASS %s_TOP_UART name=%s msg_bytes=%d cs_bytes=%d out_bytes=%d",
        vec["mode_name"], vec["name"], len(vec["msg"]), len(vec["cs"]), vec["out_len"]
    )
