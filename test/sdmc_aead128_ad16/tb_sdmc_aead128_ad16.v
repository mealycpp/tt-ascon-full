...
module tb_sdmc_aead128_ad16;
    ...
    wire [`SDMC_TOKEN_W-1:0] token4 = {1'b0, `SDMC_TOK_AD,    4'd8, 64'h3736353433323130};
    wire [`SDMC_TOKEN_W-1:0] token5 = {1'b1, `SDMC_TOK_AD,    4'd8, 64'h3f3e3d3c3b3a3938};
    ...
    wire [`SDMC_TOKEN_W-1:0] in_token =
        ...
        (token_idx == 3'd4) ? token4 : token5;

    wire in_empty = (token_idx >= 3'd6);
    wire in_pop;
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_ad16.vcd");
        $dumpvars(0, tb_sdmc_aead128_ad16);
        ...
        if (token_idx !== 3'd6) begin
            $display("FAIL tokens consumed=%0d", token_idx);
            $finish;
        end

        if (tag !== 128'hdce646b3c7d7cfc09dee3083db0c23e4) begin
            $display("FAIL tag mismatch got=%h exp=%h", tag, 128'hdce646b3c7d7cfc09dee3083db0c23e4);
            $finish;
        end

        $display("PASS sdmc_aead128_ad16");
        $finish;
    end
