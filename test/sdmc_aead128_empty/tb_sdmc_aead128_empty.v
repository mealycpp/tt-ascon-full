...
module tb_sdmc_aead128_empty;
    ...
    wire [TOKEN_W-1:0] in_token =
        ...
        (token_idx == 3'd2) ? token2 : token3;

    wire in_empty = (token_idx >= 3'd4);
    wire in_pop;
    ...
    always @(posedge clk) begin
        if (!rst_n || clear) begin
            ...
            if (out_push) begin
                if (tok_kind !== TOK_TAG || tok_bytes !== 4'd8) begin
                ...
                end else if (tag_idx == 2'd1) begin
                    tag[127:64] <= tok_data;
                    if (!tok_last) begin
                        $display("FAIL final tag missing last");
                        $finish;
                    end
                end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_empty.vcd");
        $dumpvars(0, tb_sdmc_aead128_empty);
        ...
        if (token_idx !== 3'd4) begin
            $display("FAIL tokens consumed=%0d", token_idx);
            $finish;
        end

        if (tag_idx !== 2'd2) begin
            $display("FAIL tag words emitted=%0d", tag_idx);
            $finish;
        end

        if (tag !== 128'hc62e8bee468ff66b31c9be1182279c4f) begin
            $display("FAIL tag mismatch");
            $display("got=%h", tag);
            $finish;
        end

        $display("PASS sdmc_aead128_empty");
        $finish;
    end
