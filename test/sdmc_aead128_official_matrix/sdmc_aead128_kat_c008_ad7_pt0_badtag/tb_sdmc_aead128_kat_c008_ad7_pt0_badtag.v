...
module tb_sdmc_aead128_kat_c008_ad7_pt0_badtag;
    ...
    initial begin
        ...
        token_mem[5] = {1'b0, `SDMC_TOK_TAG, 4'd8, 64'hc671e073d5bb8ab3};
        token_mem[6] = {1'b1, `SDMC_TOK_TAG, 4'd8, 64'haa658fa6c4ea5e26};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c008_ad7_pt0_badtag.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c008_ad7_pt0_badtag);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=8 mode=dec_bad AD=7 PT=0");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=8 mode=dec_bad AD=7 PT=0");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=8 mode=dec_bad", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=8 mode=dec_bad", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 8'h00) begin
            $display("FAIL output mismatch Count=8 mode=dec_bad AD=7 PT=0");
            $display("got=%h", got);
            $display("exp=%h", 8'h00);
            $finish;
        end
        ...
        if (auth_ok !== 1'b0) begin
            $display("FAIL auth mismatch got=%b exp=0 Count=8 mode=dec_bad", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c008_ad7_pt0_badtag");
        $finish;
    end
