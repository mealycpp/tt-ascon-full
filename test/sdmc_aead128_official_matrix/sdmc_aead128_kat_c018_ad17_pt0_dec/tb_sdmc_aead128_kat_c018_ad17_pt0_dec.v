...
module tb_sdmc_aead128_kat_c018_ad17_pt0_dec;
    ...
    initial begin
        ...
        token_mem[8] = {1'b1, `SDMC_TOK_TAG, 4'd8, 64'h8c0ed71d799a8344};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c018_ad17_pt0_dec.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c018_ad17_pt0_dec);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=18 mode=dec_good AD=17 PT=0");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=18 mode=dec_good AD=17 PT=0");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=18 mode=dec_good", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=18 mode=dec_good", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 8'h00) begin
            $display("FAIL output mismatch Count=18 mode=dec_good AD=17 PT=0");
            $display("got=%h", got);
            $display("exp=%h", 8'h00);
            $finish;
        end
        ...
        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=18 mode=dec_good", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c018_ad17_pt0_dec");
        $finish;
    end
