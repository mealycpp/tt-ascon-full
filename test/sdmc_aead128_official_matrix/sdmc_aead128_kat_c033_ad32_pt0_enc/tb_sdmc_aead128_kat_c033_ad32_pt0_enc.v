...
module tb_sdmc_aead128_kat_c033_ad32_pt0_enc;
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c033_ad32_pt0_enc.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c033_ad32_pt0_enc);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=33 mode=enc AD=32 PT=0");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=33 mode=enc AD=32 PT=0");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=33 mode=enc", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=33 mode=enc", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 128'hbab50b5b8c54f0a6809aad028be7c3ef) begin
            $display("FAIL output mismatch Count=33 mode=enc AD=32 PT=0");
            $display("got=%h", got);
            $display("exp=%h", 128'hbab50b5b8c54f0a6809aad028be7c3ef);
            $finish;
        end

        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=33 mode=enc", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c033_ad32_pt0_enc");
        $finish;
    end
