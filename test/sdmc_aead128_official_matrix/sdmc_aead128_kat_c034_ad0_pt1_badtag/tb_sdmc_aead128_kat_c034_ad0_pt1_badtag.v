...
module tb_sdmc_aead128_kat_c034_ad0_pt1_badtag;
    ...
    initial begin
        ...
        token_mem[4] = {1'b1, `SDMC_TOK_MSG, 4'd1, 64'h00000000000000e8};
        token_mem[5] = {1'b0, `SDMC_TOK_TAG, 4'd8, 64'hfce6d31cba6a57dd};
        token_mem[6] = {1'b1, `SDMC_TOK_TAG, 4'd8, 64'h8995b7ed2ae04d70};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c034_ad0_pt1_badtag.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c034_ad0_pt1_badtag);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=34 mode=dec_bad AD=0 PT=1");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=34 mode=dec_bad AD=0 PT=1");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=34 mode=dec_bad", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=34 mode=dec_bad", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 8'h20) begin
            $display("FAIL output mismatch Count=34 mode=dec_bad AD=0 PT=1");
            $display("got=%h", got);
            $display("exp=%h", 8'h20);
            $finish;
        end
        ...
        if (auth_ok !== 1'b0) begin
            $display("FAIL auth mismatch got=%b exp=0 Count=34 mode=dec_bad", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c034_ad0_pt1_badtag");
        $finish;
    end
