...
module tb_sdmc_aead128_kat_c034_ad0_pt1_enc;
    ...
    initial begin
        ...
        token_mem[4] = {1'b1, `SDMC_TOK_MSG, 4'd1, 64'h0000000000000020};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c034_ad0_pt1_enc.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c034_ad0_pt1_enc);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=34 mode=enc AD=0 PT=1");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=34 mode=enc AD=0 PT=1");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=34 mode=enc", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=34 mode=enc", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 136'h8895b7ed2ae04d70fce6d31cba6a57dde8) begin
            $display("FAIL output mismatch Count=34 mode=enc AD=0 PT=1");
            $display("got=%h", got);
            $display("exp=%h", 136'h8895b7ed2ae04d70fce6d31cba6a57dde8);
            $finish;
        end

        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=34 mode=enc", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c034_ad0_pt1_enc");
        $finish;
    end
