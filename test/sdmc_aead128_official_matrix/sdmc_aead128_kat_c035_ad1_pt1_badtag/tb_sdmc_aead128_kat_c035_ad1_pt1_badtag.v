...
module tb_sdmc_aead128_kat_c035_ad1_pt1_badtag;

    localparam integer TOKEN_COUNT = 8;
    localparam integer OUT_BYTES = 1;
    localparam integer OUT_BITS = 8;
    localparam integer PT_BYTES = 1;
    ...
    initial begin
        ...
        token_mem[4] = {1'b1, `SDMC_TOK_AD, 4'd1, 64'h0000000000000030};
        token_mem[5] = {1'b1, `SDMC_TOK_MSG, 4'd1, 64'h0000000000000096};
        token_mem[6] = {1'b0, `SDMC_TOK_TAG, 4'd8, 64'hd8a7756c8316802b};
        token_mem[7] = {1'b1, `SDMC_TOK_TAG, 4'd8, 64'h87d845a28c586668};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c035_ad1_pt1_badtag.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c035_ad1_pt1_badtag);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=35 mode=dec_bad AD=1 PT=1");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=35 mode=dec_bad AD=1 PT=1");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=35 mode=dec_bad", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=35 mode=dec_bad", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 8'h20) begin
            $display("FAIL output mismatch Count=35 mode=dec_bad AD=1 PT=1");
            $display("got=%h", got);
            $display("exp=%h", 8'h20);
            $finish;
        end
        ...
        if (auth_ok !== 1'b0) begin
            $display("FAIL auth mismatch got=%b exp=0 Count=35 mode=dec_bad", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c035_ad1_pt1_badtag");
        $finish;
    end
