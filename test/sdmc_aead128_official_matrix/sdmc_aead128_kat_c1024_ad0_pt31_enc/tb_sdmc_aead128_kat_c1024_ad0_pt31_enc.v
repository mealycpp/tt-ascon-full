...
module tb_sdmc_aead128_kat_c1024_ad0_pt31_enc;
    ...
    initial begin
        ...
        token_mem[7] = {1'b1, `SDMC_TOK_MSG, 4'd7, 64'h003e3d3c3b3a3938};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c1024_ad0_pt31_enc.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c1024_ad0_pt31_enc);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=1024 mode=enc AD=0 PT=31");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=1024 mode=enc AD=0 PT=31");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=1024 mode=enc", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=1024 mode=enc", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 376'hebc253dd02b7f56239226a999c5b9f605466001f2d0f970703e8153eaa8960bba297383172e8e3eac56c24eedec3e8) begin
            $display("FAIL output mismatch Count=1024 mode=enc AD=0 PT=31");
            $display("got=%h", got);
            $display("exp=%h", 376'hebc253dd02b7f56239226a999c5b9f605466001f2d0f970703e8153eaa8960bba297383172e8e3eac56c24eedec3e8);
            $finish;
        end

        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=1024 mode=enc", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c1024_ad0_pt31_enc");
        $finish;
    end
