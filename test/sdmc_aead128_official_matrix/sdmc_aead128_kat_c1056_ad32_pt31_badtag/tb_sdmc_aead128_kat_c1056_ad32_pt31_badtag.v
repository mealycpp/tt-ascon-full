...
module tb_sdmc_aead128_kat_c1056_ad32_pt31_badtag;
    ...
    initial begin
        ...
        token_mem[8] = {1'b0, `SDMC_TOK_MSG, 4'd8, 64'hbf6da66046d034cb};
        token_mem[9] = {1'b0, `SDMC_TOK_MSG, 4'd8, 64'haab8f50166859cbe};
        token_mem[10] = {1'b0, `SDMC_TOK_MSG, 4'd8, 64'hfbf7c85ab599a451};
        token_mem[11] = {1'b1, `SDMC_TOK_MSG, 4'd7, 64'h00e93e611a33bcef};
        token_mem[12] = {1'b0, `SDMC_TOK_TAG, 4'd8, 64'h1fbf8198e0d50007};
        token_mem[13] = {1'b1, `SDMC_TOK_TAG, 4'd8, 64'h779aebf3e66d949a};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c1056_ad32_pt31_badtag.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c1056_ad32_pt31_badtag);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=1056 mode=dec_bad AD=32 PT=31");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=1056 mode=dec_bad AD=32 PT=31");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=1056 mode=dec_bad", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=1056 mode=dec_bad", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 248'h3e3d3c3b3a393837363534333231302f2e2d2c2b2a29282726252423222120) begin
            $display("FAIL output mismatch Count=1056 mode=dec_bad AD=32 PT=31");
            $display("got=%h", got);
        ...
        if (auth_ok !== 1'b0) begin
            $display("FAIL auth mismatch got=%b exp=0 Count=1056 mode=dec_bad", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c1056_ad32_pt31_badtag");
        $finish;
    end
