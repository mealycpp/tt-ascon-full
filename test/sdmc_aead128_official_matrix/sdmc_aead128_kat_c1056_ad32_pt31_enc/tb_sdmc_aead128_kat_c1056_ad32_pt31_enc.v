...
module tb_sdmc_aead128_kat_c1056_ad32_pt31_enc;

    localparam integer TOKEN_COUNT = 12;
    localparam integer OUT_BYTES = 47;
    localparam integer OUT_BITS = 376;
    ...
    initial begin
        ...
        token_mem[7] = {1'b1, `SDMC_TOK_AD, 4'd8, 64'h4f4e4d4c4b4a4948};
        ...
        token_mem[11] = {1'b1, `SDMC_TOK_MSG, 4'd7, 64'h003e3d3c3b3a3938};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c1056_ad32_pt31_enc.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c1056_ad32_pt31_enc);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=1056 mode=enc AD=32 PT=31");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=1056 mode=enc AD=32 PT=31");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=1056 mode=enc", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=1056 mode=enc", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 376'h769aebf3e66d949a1fbf8198e0d50007e93e611a33bceffbf7c85ab599a451aab8f50166859cbebf6da66046d034cb) begin
            $display("FAIL output mismatch Count=1056 mode=enc AD=32 PT=31");
            $display("got=%h", got);
            $display("exp=%h", 376'h769aebf3e66d949a1fbf8198e0d50007e93e611a33bceffbf7c85ab599a451aab8f50166859cbebf6da66046d034cb);
            $finish;
        end

        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=1056 mode=enc", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c1056_ad32_pt31_enc");
        $finish;
    end
