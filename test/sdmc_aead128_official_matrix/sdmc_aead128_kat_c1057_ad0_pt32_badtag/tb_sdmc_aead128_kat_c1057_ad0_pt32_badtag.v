...
module tb_sdmc_aead128_kat_c1057_ad0_pt32_badtag;
    ...
    initial begin
        ...
        token_mem[7] = {1'b1, `SDMC_TOK_MSG, 4'd8, 64'hc25466001f2d0f97};
        token_mem[8] = {1'b0, `SDMC_TOK_TAG, 4'd8, 64'h7df0b92c17faa5aa};
        token_mem[9] = {1'b1, `SDMC_TOK_TAG, 4'd8, 64'hc00b44c7ef3c4607};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c1057_ad0_pt32_badtag.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c1057_ad0_pt32_badtag);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=1057 mode=dec_bad AD=0 PT=32");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=1057 mode=dec_bad AD=0 PT=32");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=1057 mode=dec_bad", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=1057 mode=dec_bad", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 256'h3f3e3d3c3b3a393837363534333231302f2e2d2c2b2a29282726252423222120) begin
            $display("FAIL output mismatch Count=1057 mode=dec_bad AD=0 PT=32");
            $display("got=%h", got);
        ...
        if (auth_ok !== 1'b0) begin
            $display("FAIL auth mismatch got=%b exp=0 Count=1057 mode=dec_bad", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c1057_ad0_pt32_badtag");
        $finish;
    end
