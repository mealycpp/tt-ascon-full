...
module tb_sdmc_aead128_kat_c1057_ad0_pt32_enc;

    localparam integer TOKEN_COUNT = 8;
    ...
    initial begin
        ...
        token_mem[4] = {1'b0, `SDMC_TOK_MSG, 4'd8, 64'h2726252423222120};
        token_mem[5] = {1'b0, `SDMC_TOK_MSG, 4'd8, 64'h2f2e2d2c2b2a2928};
        token_mem[6] = {1'b0, `SDMC_TOK_MSG, 4'd8, 64'h3736353433323130};
        token_mem[7] = {1'b1, `SDMC_TOK_MSG, 4'd8, 64'h3f3e3d3c3b3a3938};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c1057_ad0_pt32_enc.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c1057_ad0_pt32_enc);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=1057 mode=enc AD=0 PT=32");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=1057 mode=enc AD=0 PT=32");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=1057 mode=enc", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=1057 mode=enc", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 384'hc10b44c7ef3c46077df0b92c17faa5aac25466001f2d0f970703e8153eaa8960bba297383172e8e3eac56c24eedec3e8) begin
            $display("FAIL output mismatch Count=1057 mode=enc AD=0 PT=32");
            $display("got=%h", got);
            $display("exp=%h", 384'hc10b44c7ef3c46077df0b92c17faa5aac25466001f2d0f970703e8153eaa8960bba297383172e8e3eac56c24eedec3e8);
            $finish;
        end

        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=1057 mode=enc", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c1057_ad0_pt32_enc");
        $finish;
    end
