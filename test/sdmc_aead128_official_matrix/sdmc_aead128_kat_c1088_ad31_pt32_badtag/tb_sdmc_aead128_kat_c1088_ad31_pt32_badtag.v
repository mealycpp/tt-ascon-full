...
module tb_sdmc_aead128_kat_c1088_ad31_pt32_badtag;

    localparam integer TOKEN_COUNT = 14;
    localparam integer OUT_BYTES = 32;
    localparam integer OUT_BITS = 256;
    localparam integer PT_BYTES = 32;
    ...
    initial begin
        ...
        token_mem[5] = {1'b0, `SDMC_TOK_AD, 4'd8, 64'h3f3e3d3c3b3a3938};
        token_mem[6] = {1'b0, `SDMC_TOK_AD, 4'd8, 64'h4746454443424140};
        token_mem[7] = {1'b1, `SDMC_TOK_AD, 4'd7, 64'h004e4d4c4b4a4948};
        token_mem[8] = {1'b0, `SDMC_TOK_MSG, 4'd8, 64'hbb0c0ea65f2e394b};
        token_mem[9] = {1'b0, `SDMC_TOK_MSG, 4'd8, 64'hbd62326eb97d54ca};
        token_mem[10] = {1'b0, `SDMC_TOK_MSG, 4'd8, 64'h4fe208e6c0d68283};
        token_mem[11] = {1'b1, `SDMC_TOK_MSG, 4'd8, 64'hade52647fcaa1a44};
        token_mem[12] = {1'b0, `SDMC_TOK_TAG, 4'd8, 64'h0b5ee20ada339277};
        token_mem[13] = {1'b1, `SDMC_TOK_TAG, 4'd8, 64'hc36e8905486e85de};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c1088_ad31_pt32_badtag.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c1088_ad31_pt32_badtag);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=1088 mode=dec_bad AD=31 PT=32");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=1088 mode=dec_bad AD=31 PT=32");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=1088 mode=dec_bad", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=1088 mode=dec_bad", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 256'h3f3e3d3c3b3a393837363534333231302f2e2d2c2b2a29282726252423222120) begin
            $display("FAIL output mismatch Count=1088 mode=dec_bad AD=31 PT=32");
            $display("got=%h", got);
            $display("exp=%h", 256'h3f3e3d3c3b3a393837363534333231302f2e2d2c2b2a29282726252423222120);
            $finish;
        end

        if (auth_ok !== 1'b0) begin
            $display("FAIL auth mismatch got=%b exp=0 Count=1088 mode=dec_bad", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c1088_ad31_pt32_badtag");
        $finish;
    end
