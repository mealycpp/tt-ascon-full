...
module tb_sdmc_aead128_kat_c232_ad0_pt7_badtag;
    ...
    initial begin
        ...
        token_mem[4] = {1'b1, `SDMC_TOK_MSG, 4'd7, 64'h00c56c24eedec3e8};
        token_mem[5] = {1'b0, `SDMC_TOK_TAG, 4'd8, 64'he96e45d2724f3b65};
        token_mem[6] = {1'b1, `SDMC_TOK_TAG, 4'd8, 64'h7bf8a3777605dadb};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c232_ad0_pt7_badtag.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c232_ad0_pt7_badtag);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=232 mode=dec_bad AD=0 PT=7");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=232 mode=dec_bad AD=0 PT=7");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=232 mode=dec_bad", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=232 mode=dec_bad", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 56'h26252423222120) begin
            $display("FAIL output mismatch Count=232 mode=dec_bad AD=0 PT=7");
            $display("got=%h", got);
        ...
        if (auth_ok !== 1'b0) begin
            $display("FAIL auth mismatch got=%b exp=0 Count=232 mode=dec_bad", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c232_ad0_pt7_badtag");
        $finish;
    end
