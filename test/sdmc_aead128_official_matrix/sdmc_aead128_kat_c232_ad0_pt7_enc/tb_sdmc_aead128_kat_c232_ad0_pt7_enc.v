...
module tb_sdmc_aead128_kat_c232_ad0_pt7_enc;
    ...
    initial begin
        ...
        token_mem[4] = {1'b1, `SDMC_TOK_MSG, 4'd7, 64'h0026252423222120};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c232_ad0_pt7_enc.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c232_ad0_pt7_enc);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=232 mode=enc AD=0 PT=7");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=232 mode=enc AD=0 PT=7");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=232 mode=enc", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=232 mode=enc", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 184'h7af8a3777605dadbe96e45d2724f3b65c56c24eedec3e8) begin
            $display("FAIL output mismatch Count=232 mode=enc AD=0 PT=7");
            $display("got=%h", got);
            $display("exp=%h", 184'h7af8a3777605dadbe96e45d2724f3b65c56c24eedec3e8);
            $finish;
        end

        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=232 mode=enc", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c232_ad0_pt7_enc");
        $finish;
    end
