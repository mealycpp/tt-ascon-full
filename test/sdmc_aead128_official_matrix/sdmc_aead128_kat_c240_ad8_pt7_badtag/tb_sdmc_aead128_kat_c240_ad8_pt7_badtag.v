...
module tb_sdmc_aead128_kat_c240_ad8_pt7_badtag;

    localparam integer TOKEN_COUNT = 8;
    localparam integer OUT_BYTES = 7;
    localparam integer OUT_BITS = 56;
    localparam integer PT_BYTES = 7;
    ...
    initial begin
        ...
        token_mem[5] = {1'b1, `SDMC_TOK_MSG, 4'd7, 64'h00a6ebe7b12cbefa};
        token_mem[6] = {1'b0, `SDMC_TOK_TAG, 4'd8, 64'hf1ca2a5a4fb9fe0f};
        token_mem[7] = {1'b1, `SDMC_TOK_TAG, 4'd8, 64'h51efda28fa5a5b5a};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c240_ad8_pt7_badtag.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c240_ad8_pt7_badtag);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=240 mode=dec_bad AD=8 PT=7");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=240 mode=dec_bad AD=8 PT=7");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=240 mode=dec_bad", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=240 mode=dec_bad", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 56'h26252423222120) begin
            $display("FAIL output mismatch Count=240 mode=dec_bad AD=8 PT=7");
            $display("got=%h", got);
            $display("exp=%h", 56'h26252423222120);
            $finish;
        end

        if (auth_ok !== 1'b0) begin
            $display("FAIL auth mismatch got=%b exp=0 Count=240 mode=dec_bad", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c240_ad8_pt7_badtag");
        $finish;
    end
