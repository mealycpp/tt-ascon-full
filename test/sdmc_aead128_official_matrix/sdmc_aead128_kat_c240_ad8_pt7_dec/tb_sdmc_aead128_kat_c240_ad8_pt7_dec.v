...
module tb_sdmc_aead128_kat_c240_ad8_pt7_dec;
    ...
    initial begin
        ...
        token_mem[7] = {1'b1, `SDMC_TOK_TAG, 4'd8, 64'h50efda28fa5a5b5a};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c240_ad8_pt7_dec.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c240_ad8_pt7_dec);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=240 mode=dec_good AD=8 PT=7");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=240 mode=dec_good AD=8 PT=7");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=240 mode=dec_good", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=240 mode=dec_good", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 56'h26252423222120) begin
            $display("FAIL output mismatch Count=240 mode=dec_good AD=8 PT=7");
            $display("got=%h", got);
        ...
        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=240 mode=dec_good", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c240_ad8_pt7_dec");
        $finish;
    end
