...
module tb_sdmc_aead128_kat_c240_ad8_pt7_enc;

    localparam integer TOKEN_COUNT = 6;
    localparam integer OUT_BYTES = 23;
    localparam integer OUT_BITS = 184;
    ...
    initial begin
        ...
        token_mem[5] = {1'b1, `SDMC_TOK_MSG, 4'd7, 64'h0026252423222120};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c240_ad8_pt7_enc.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c240_ad8_pt7_enc);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=240 mode=enc AD=8 PT=7");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=240 mode=enc AD=8 PT=7");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=240 mode=enc", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=240 mode=enc", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 184'h50efda28fa5a5b5af1ca2a5a4fb9fe0fa6ebe7b12cbefa) begin
            $display("FAIL output mismatch Count=240 mode=enc AD=8 PT=7");
            $display("got=%h", got);
            $display("exp=%h", 184'h50efda28fa5a5b5af1ca2a5a4fb9fe0fa6ebe7b12cbefa);
            $finish;
        end

        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=240 mode=enc", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c240_ad8_pt7_enc");
        $finish;
    end
