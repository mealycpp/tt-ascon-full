...
module tb_sdmc_aead128_kat_c265_ad0_pt8_badtag;
    ...
    initial begin
        ...
        token_mem[4] = {1'b1, `SDMC_TOK_MSG, 4'd8, 64'heac56c24eedec3e8};
        token_mem[5] = {1'b0, `SDMC_TOK_TAG, 4'd8, 64'ha382b7336bef55e4};
        token_mem[6] = {1'b1, `SDMC_TOK_TAG, 4'd8, 64'h263c379566ed91dd};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c265_ad0_pt8_badtag.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c265_ad0_pt8_badtag);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=265 mode=dec_bad AD=0 PT=8");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=265 mode=dec_bad AD=0 PT=8");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=265 mode=dec_bad", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=265 mode=dec_bad", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 64'h2726252423222120) begin
            $display("FAIL output mismatch Count=265 mode=dec_bad AD=0 PT=8");
            $display("got=%h", got);
        ...
        if (auth_ok !== 1'b0) begin
            $display("FAIL auth mismatch got=%b exp=0 Count=265 mode=dec_bad", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c265_ad0_pt8_badtag");
        $finish;
    end
