...
module tb_sdmc_aead128_kat_c265_ad0_pt8_enc;
    ...
    initial begin
        ...
        token_mem[4] = {1'b1, `SDMC_TOK_MSG, 4'd8, 64'h2726252423222120};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c265_ad0_pt8_enc.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c265_ad0_pt8_enc);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=265 mode=enc AD=0 PT=8");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=265 mode=enc AD=0 PT=8");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=265 mode=enc", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=265 mode=enc", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 192'h273c379566ed91dda382b7336bef55e4eac56c24eedec3e8) begin
            $display("FAIL output mismatch Count=265 mode=enc AD=0 PT=8");
            $display("got=%h", got);
            $display("exp=%h", 192'h273c379566ed91dda382b7336bef55e4eac56c24eedec3e8);
            $finish;
        end

        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=265 mode=enc", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c265_ad0_pt8_enc");
        $finish;
    end
