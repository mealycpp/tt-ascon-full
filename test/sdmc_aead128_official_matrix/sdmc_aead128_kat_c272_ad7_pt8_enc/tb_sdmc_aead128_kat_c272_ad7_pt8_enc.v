...
module tb_sdmc_aead128_kat_c272_ad7_pt8_enc;
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c272_ad7_pt8_enc.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c272_ad7_pt8_enc);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=272 mode=enc AD=7 PT=8");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=272 mode=enc AD=7 PT=8");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=272 mode=enc", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=272 mode=enc", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 192'h6ea9c3c92e28246904c3af9c8be3f19ceb86f303d44b026e) begin
            $display("FAIL output mismatch Count=272 mode=enc AD=7 PT=8");
            $display("got=%h", got);
            $display("exp=%h", 192'h6ea9c3c92e28246904c3af9c8be3f19ceb86f303d44b026e);
            $finish;
        end

        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=272 mode=enc", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c272_ad7_pt8_enc");
        $finish;
    end
