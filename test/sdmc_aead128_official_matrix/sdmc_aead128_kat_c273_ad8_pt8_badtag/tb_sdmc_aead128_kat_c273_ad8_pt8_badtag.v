...
module tb_sdmc_aead128_kat_c273_ad8_pt8_badtag;

    localparam integer TOKEN_COUNT = 8;
    localparam integer OUT_BYTES = 8;
    localparam integer OUT_BITS = 64;
    localparam integer PT_BYTES = 8;
    ...
    initial begin
        ...
        token_mem[4] = {1'b1, `SDMC_TOK_AD, 4'd8, 64'h3736353433323130};
        token_mem[5] = {1'b1, `SDMC_TOK_MSG, 4'd8, 64'h32a6ebe7b12cbefa};
        token_mem[6] = {1'b0, `SDMC_TOK_TAG, 4'd8, 64'h34258738c34b5240};
        token_mem[7] = {1'b1, `SDMC_TOK_TAG, 4'd8, 64'hc4de427053c3841b};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c273_ad8_pt8_badtag.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c273_ad8_pt8_badtag);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=273 mode=dec_bad AD=8 PT=8");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=273 mode=dec_bad AD=8 PT=8");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=273 mode=dec_bad", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=273 mode=dec_bad", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 64'h2726252423222120) begin
            $display("FAIL output mismatch Count=273 mode=dec_bad AD=8 PT=8");
            $display("got=%h", got);
            $display("exp=%h", 64'h2726252423222120);
            $finish;
        end

        if (auth_ok !== 1'b0) begin
            $display("FAIL auth mismatch got=%b exp=0 Count=273 mode=dec_bad", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c273_ad8_pt8_badtag");
        $finish;
    end
