...
module tb_sdmc_aead128_kat_c273_ad8_pt8_enc;

    localparam integer TOKEN_COUNT = 6;
    localparam integer OUT_BYTES = 24;
    localparam integer OUT_BITS = 192;
    ...
    initial begin
        ...
        token_mem[5] = {1'b1, `SDMC_TOK_MSG, 4'd8, 64'h2726252423222120};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c273_ad8_pt8_enc.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c273_ad8_pt8_enc);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=273 mode=enc AD=8 PT=8");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=273 mode=enc AD=8 PT=8");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=273 mode=enc", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=273 mode=enc", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 192'hc5de427053c3841b34258738c34b524032a6ebe7b12cbefa) begin
            $display("FAIL output mismatch Count=273 mode=enc AD=8 PT=8");
            $display("got=%h", got);
            $display("exp=%h", 192'hc5de427053c3841b34258738c34b524032a6ebe7b12cbefa);
            $finish;
        end

        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=273 mode=enc", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c273_ad8_pt8_enc");
        $finish;
    end
