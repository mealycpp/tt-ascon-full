...
module tb_sdmc_aead128_kat_c298_ad0_pt9_badtag;

    localparam integer TOKEN_COUNT = 8;
    localparam integer OUT_BYTES = 9;
    localparam integer OUT_BITS = 72;
    localparam integer PT_BYTES = 9;
    ...
    initial begin
        ...
        token_mem[5] = {1'b1, `SDMC_TOK_MSG, 4'd1, 64'h00000000000000e3};
        token_mem[6] = {1'b0, `SDMC_TOK_TAG, 4'd8, 64'h921b10ae50c99b32};
        token_mem[7] = {1'b1, `SDMC_TOK_TAG, 4'd8, 64'h6dbfdcbe5e60f347};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c298_ad0_pt9_badtag.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c298_ad0_pt9_badtag);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=298 mode=dec_bad AD=0 PT=9");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=298 mode=dec_bad AD=0 PT=9");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=298 mode=dec_bad", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=298 mode=dec_bad", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 72'h282726252423222120) begin
            $display("FAIL output mismatch Count=298 mode=dec_bad AD=0 PT=9");
            $display("got=%h", got);
            $display("exp=%h", 72'h282726252423222120);
            $finish;
        end

        if (auth_ok !== 1'b0) begin
            $display("FAIL auth mismatch got=%b exp=0 Count=298 mode=dec_bad", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c298_ad0_pt9_badtag");
        $finish;
    end
