...
module tb_sdmc_aead128_kat_c298_ad0_pt9_enc;

    localparam integer TOKEN_COUNT = 6;
    localparam integer OUT_BYTES = 25;
    localparam integer OUT_BITS = 200;
    ...
    initial begin
        ...
        token_mem[5] = {1'b1, `SDMC_TOK_MSG, 4'd1, 64'h0000000000000028};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c298_ad0_pt9_enc.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c298_ad0_pt9_enc);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=298 mode=enc AD=0 PT=9");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=298 mode=enc AD=0 PT=9");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=298 mode=enc", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=298 mode=enc", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 200'h6cbfdcbe5e60f347921b10ae50c99b32e3eac56c24eedec3e8) begin
            $display("FAIL output mismatch Count=298 mode=enc AD=0 PT=9");
            $display("got=%h", got);
            $display("exp=%h", 200'h6cbfdcbe5e60f347921b10ae50c99b32e3eac56c24eedec3e8);
            $finish;
        end

        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=298 mode=enc", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c298_ad0_pt9_enc");
        $finish;
    end
