...
module tb_sdmc_aead128_kat_c496_ad0_pt15_badtag;

    localparam integer TOKEN_COUNT = 8;
    localparam integer OUT_BYTES = 15;
    localparam integer OUT_BITS = 120;
    localparam integer PT_BYTES = 15;
    ...
    initial begin
        ...
        token_mem[5] = {1'b1, `SDMC_TOK_MSG, 4'd7, 64'h00a297383172e8e3};
        token_mem[6] = {1'b0, `SDMC_TOK_TAG, 4'd8, 64'h524a83a01dccae83};
        token_mem[7] = {1'b1, `SDMC_TOK_TAG, 4'd8, 64'h0564dbcdbfc40e94};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c496_ad0_pt15_badtag.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c496_ad0_pt15_badtag);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=496 mode=dec_bad AD=0 PT=15");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=496 mode=dec_bad AD=0 PT=15");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=496 mode=dec_bad", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=496 mode=dec_bad", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 120'h2e2d2c2b2a29282726252423222120) begin
            $display("FAIL output mismatch Count=496 mode=dec_bad AD=0 PT=15");
            $display("got=%h", got);
            $display("exp=%h", 120'h2e2d2c2b2a29282726252423222120);
            $finish;
        end

        if (auth_ok !== 1'b0) begin
            $display("FAIL auth mismatch got=%b exp=0 Count=496 mode=dec_bad", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c496_ad0_pt15_badtag");
        $finish;
    end
