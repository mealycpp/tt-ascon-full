...
module tb_sdmc_aead128_kat_c496_ad0_pt15_enc;
    ...
    initial begin
        ...
        token_mem[5] = {1'b1, `SDMC_TOK_MSG, 4'd7, 64'h002e2d2c2b2a2928};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c496_ad0_pt15_enc.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c496_ad0_pt15_enc);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=496 mode=enc AD=0 PT=15");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=496 mode=enc AD=0 PT=15");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=496 mode=enc", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=496 mode=enc", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 248'h0464dbcdbfc40e94524a83a01dccae83a297383172e8e3eac56c24eedec3e8) begin
            $display("FAIL output mismatch Count=496 mode=enc AD=0 PT=15");
            $display("got=%h", got);
            $display("exp=%h", 248'h0464dbcdbfc40e94524a83a01dccae83a297383172e8e3eac56c24eedec3e8);
            $finish;
        end

        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=496 mode=enc", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c496_ad0_pt15_enc");
        $finish;
    end
