...
module tb_sdmc_aead128_kat_c512_ad16_pt15_badtag;
    ...
    initial begin
        ...
        token_mem[7] = {1'b1, `SDMC_TOK_MSG, 4'd7, 64'h003ec199f30c09ac};
        token_mem[8] = {1'b0, `SDMC_TOK_TAG, 4'd8, 64'h7704040bc8e06b64};
        token_mem[9] = {1'b1, `SDMC_TOK_TAG, 4'd8, 64'h498917d648eb4103};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c512_ad16_pt15_badtag.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c512_ad16_pt15_badtag);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=512 mode=dec_bad AD=16 PT=15");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=512 mode=dec_bad AD=16 PT=15");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=512 mode=dec_bad", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=512 mode=dec_bad", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 120'h2e2d2c2b2a29282726252423222120) begin
            $display("FAIL output mismatch Count=512 mode=dec_bad AD=16 PT=15");
            $display("got=%h", got);
        ...
        if (auth_ok !== 1'b0) begin
            $display("FAIL auth mismatch got=%b exp=0 Count=512 mode=dec_bad", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c512_ad16_pt15_badtag");
        $finish;
    end
