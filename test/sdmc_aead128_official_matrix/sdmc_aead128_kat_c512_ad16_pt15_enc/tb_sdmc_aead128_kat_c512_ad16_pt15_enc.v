...
module tb_sdmc_aead128_kat_c512_ad16_pt15_enc;
    ...
    initial begin
        ...
        token_mem[7] = {1'b1, `SDMC_TOK_MSG, 4'd7, 64'h002e2d2c2b2a2928};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c512_ad16_pt15_enc.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c512_ad16_pt15_enc);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=512 mode=enc AD=16 PT=15");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=512 mode=enc AD=16 PT=15");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=512 mode=enc", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=512 mode=enc", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 248'h488917d648eb41037704040bc8e06b643ec199f30c09ac9b7ce98bb2eb7363) begin
            $display("FAIL output mismatch Count=512 mode=enc AD=16 PT=15");
            $display("got=%h", got);
            $display("exp=%h", 248'h488917d648eb41037704040bc8e06b643ec199f30c09ac9b7ce98bb2eb7363);
            $finish;
        end

        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=512 mode=enc", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c512_ad16_pt15_enc");
        $finish;
    end
