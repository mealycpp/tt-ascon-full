...
module tb_sdmc_aead128_kat_c529_ad0_pt16_dec;
    ...
    initial begin
        ...
        token_mem[7] = {1'b1, `SDMC_TOK_TAG, 4'd8, 64'ha7276dd4248f0477};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c529_ad0_pt16_dec.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c529_ad0_pt16_dec);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=529 mode=dec_good AD=0 PT=16");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=529 mode=dec_good AD=0 PT=16");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=529 mode=dec_good", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=529 mode=dec_good", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 128'h2f2e2d2c2b2a29282726252423222120) begin
            $display("FAIL output mismatch Count=529 mode=dec_good AD=0 PT=16");
            $display("got=%h", got);
        ...
        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=529 mode=dec_good", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c529_ad0_pt16_dec");
        $finish;
    end
