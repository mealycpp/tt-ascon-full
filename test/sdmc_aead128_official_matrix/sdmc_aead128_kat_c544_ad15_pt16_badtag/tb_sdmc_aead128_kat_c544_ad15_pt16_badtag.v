...
module tb_sdmc_aead128_kat_c544_ad15_pt16_badtag;
    ...
    initial begin
        ...
        token_mem[6] = {1'b0, `SDMC_TOK_MSG, 4'd8, 64'hc45c1abcda19fd20};
        token_mem[7] = {1'b1, `SDMC_TOK_MSG, 4'd8, 64'h1360ac4dd321a649};
        token_mem[8] = {1'b0, `SDMC_TOK_TAG, 4'd8, 64'ha7c46cd4b78b2472};
        token_mem[9] = {1'b1, `SDMC_TOK_TAG, 4'd8, 64'hb7552278abc1f9f5};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c544_ad15_pt16_badtag.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c544_ad15_pt16_badtag);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=544 mode=dec_bad AD=15 PT=16");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=544 mode=dec_bad AD=15 PT=16");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=544 mode=dec_bad", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=544 mode=dec_bad", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 128'h2f2e2d2c2b2a29282726252423222120) begin
            $display("FAIL output mismatch Count=544 mode=dec_bad AD=15 PT=16");
            $display("got=%h", got);
        ...
        if (auth_ok !== 1'b0) begin
            $display("FAIL auth mismatch got=%b exp=0 Count=544 mode=dec_bad", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c544_ad15_pt16_badtag");
        $finish;
    end
