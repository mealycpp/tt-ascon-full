...
module tb_sdmc_aead128_kat_c544_ad15_pt16_enc;
    ...
    initial begin
        ...
        token_mem[5] = {1'b1, `SDMC_TOK_AD, 4'd7, 64'h003e3d3c3b3a3938};
        token_mem[6] = {1'b0, `SDMC_TOK_MSG, 4'd8, 64'h2726252423222120};
        token_mem[7] = {1'b1, `SDMC_TOK_MSG, 4'd8, 64'h2f2e2d2c2b2a2928};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c544_ad15_pt16_enc.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c544_ad15_pt16_enc);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=544 mode=enc AD=15 PT=16");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=544 mode=enc AD=15 PT=16");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=544 mode=enc", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=544 mode=enc", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 256'hb6552278abc1f9f5a7c46cd4b78b24721360ac4dd321a649c45c1abcda19fd20) begin
            $display("FAIL output mismatch Count=544 mode=enc AD=15 PT=16");
            $display("got=%h", got);
            $display("exp=%h", 256'hb6552278abc1f9f5a7c46cd4b78b24721360ac4dd321a649c45c1abcda19fd20);
            $finish;
        end

        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=544 mode=enc", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c544_ad15_pt16_enc");
        $finish;
    end
