...
module tb_sdmc_aead128_kat_c545_ad16_pt16_enc;
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c545_ad16_pt16_enc.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c545_ad16_pt16_enc);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=545 mode=enc AD=16 PT=16");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=545 mode=enc AD=16 PT=16");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=545 mode=enc", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=545 mode=enc", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 256'h592cf3134d81904c84f4e809d2c0bf3af13ec199f30c09ac9b7ce98bb2eb7363) begin
            $display("FAIL output mismatch Count=545 mode=enc AD=16 PT=16");
            $display("got=%h", got);
            $display("exp=%h", 256'h592cf3134d81904c84f4e809d2c0bf3af13ec199f30c09ac9b7ce98bb2eb7363);
            $finish;
        end

        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=545 mode=enc", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c545_ad16_pt16_enc");
        $finish;
    end
