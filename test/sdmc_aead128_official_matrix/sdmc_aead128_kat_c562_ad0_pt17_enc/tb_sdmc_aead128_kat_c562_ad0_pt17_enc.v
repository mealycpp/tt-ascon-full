...
module tb_sdmc_aead128_kat_c562_ad0_pt17_enc;

    localparam integer TOKEN_COUNT = 7;
    ...
    initial begin
        ...
        token_mem[6] = {1'b1, `SDMC_TOK_MSG, 4'd1, 64'h0000000000000030};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c562_ad0_pt17_enc.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c562_ad0_pt17_enc);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=562 mode=enc AD=0 PT=17");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=562 mode=enc AD=0 PT=17");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=562 mode=enc", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=562 mode=enc", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 264'h4438b2b3ea0c0bdd7562459d5302103060bba297383172e8e3eac56c24eedec3e8) begin
            $display("FAIL output mismatch Count=562 mode=enc AD=0 PT=17");
            $display("got=%h", got);
            $display("exp=%h", 264'h4438b2b3ea0c0bdd7562459d5302103060bba297383172e8e3eac56c24eedec3e8);
            $finish;
        end

        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=562 mode=enc", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c562_ad0_pt17_enc");
        $finish;
    end
