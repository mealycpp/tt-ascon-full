...
module tb_sdmc_aead128_kat_c579_ad17_pt17_badtag;

    localparam integer TOKEN_COUNT = 12;
    localparam integer OUT_BYTES = 17;
    localparam integer OUT_BITS = 136;
    localparam integer PT_BYTES = 17;
    ...
    initial begin
        ...
        token_mem[6] = {1'b1, `SDMC_TOK_AD, 4'd1, 64'h0000000000000040};
        token_mem[7] = {1'b0, `SDMC_TOK_MSG, 4'd8, 64'hc5f1e93d1bc777bf};
        token_mem[8] = {1'b0, `SDMC_TOK_MSG, 4'd8, 64'h9be8083a27ef72b3};
        token_mem[9] = {1'b1, `SDMC_TOK_MSG, 4'd1, 64'h00000000000000e9};
        token_mem[10] = {1'b0, `SDMC_TOK_TAG, 4'd8, 64'h79e9aec2b3d707d5};
        token_mem[11] = {1'b1, `SDMC_TOK_TAG, 4'd8, 64'h34660d97f791e711};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c579_ad17_pt17_badtag.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c579_ad17_pt17_badtag);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=579 mode=dec_bad AD=17 PT=17");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=579 mode=dec_bad AD=17 PT=17");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=579 mode=dec_bad", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=579 mode=dec_bad", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 136'h302f2e2d2c2b2a29282726252423222120) begin
            $display("FAIL output mismatch Count=579 mode=dec_bad AD=17 PT=17");
            $display("got=%h", got);
            $display("exp=%h", 136'h302f2e2d2c2b2a29282726252423222120);
            $finish;
        end

        if (auth_ok !== 1'b0) begin
            $display("FAIL auth mismatch got=%b exp=0 Count=579 mode=dec_bad", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c579_ad17_pt17_badtag");
        $finish;
    end
