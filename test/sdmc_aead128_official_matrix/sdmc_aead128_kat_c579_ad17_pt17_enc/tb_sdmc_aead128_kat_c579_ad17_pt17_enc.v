...
module tb_sdmc_aead128_kat_c579_ad17_pt17_enc;

    localparam integer TOKEN_COUNT = 10;
    localparam integer OUT_BYTES = 33;
    localparam integer OUT_BITS = 264;
    ...
    initial begin
        ...
        token_mem[7] = {1'b0, `SDMC_TOK_MSG, 4'd8, 64'h2726252423222120};
        token_mem[8] = {1'b0, `SDMC_TOK_MSG, 4'd8, 64'h2f2e2d2c2b2a2928};
        token_mem[9] = {1'b1, `SDMC_TOK_MSG, 4'd1, 64'h0000000000000030};
    end
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_kat_c579_ad17_pt17_enc.vcd");
        $dumpvars(0, tb_sdmc_aead128_kat_c579_ad17_pt17_enc);
        ...
        while (!done) begin
            ...
            if (guard > 50000) begin
                $display("FAIL timeout Count=579 mode=enc AD=17 PT=17");
                $finish;
            end
        end
        ...
        if (error) begin
            $display("FAIL error asserted Count=579 mode=enc AD=17 PT=17");
            $finish;
        end

        if (token_idx !== TOKEN_COUNT) begin
            $display("FAIL tokens consumed=%0d expected=%0d Count=579 mode=enc", token_idx, TOKEN_COUNT);
            $finish;
        end

        if (out_bytes_seen !== OUT_BYTES) begin
            $display("FAIL output bytes=%0d expected=%0d Count=579 mode=enc", out_bytes_seen, OUT_BYTES);
            $finish;
        end

        if (got !== 264'h35660d97f791e71179e9aec2b3d707d5e99be8083a27ef72b3c5f1e93d1bc777bf) begin
            $display("FAIL output mismatch Count=579 mode=enc AD=17 PT=17");
            $display("got=%h", got);
            $display("exp=%h", 264'h35660d97f791e71179e9aec2b3d707d5e99be8083a27ef72b3c5f1e93d1bc777bf);
            $finish;
        end

        if (auth_ok !== 1'b1) begin
            $display("FAIL auth mismatch got=%b exp=1 Count=579 mode=enc", auth_ok);
            $finish;
        end

        $display("PASS sdmc_aead128_kat_c579_ad17_pt17_enc");
        $finish;
    end
