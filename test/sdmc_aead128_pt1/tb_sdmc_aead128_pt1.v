...
module tb_sdmc_aead128_pt1;

    localparam TOKEN_W = `SDMC_TOKEN_W;
    localparam OUT_BYTES = 17;
    ...
    wire [TOKEN_W-1:0] token4 = {1'b1, `SDMC_TOK_MSG,   4'd1, 64'h0000000000000020};
    ...
    always @(posedge clk) begin
        if (!rst_n || clear) begin
            ...
            if (out_idx == 6'd0) begin
                if (tok_kind !== `SDMC_TOK_OUT || tok_bytes !== 4'd1) begin
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_pt1.vcd");
        $dumpvars(0, tb_sdmc_aead128_pt1);
        ...
        if (enc_out !== 136'h8895b7ed2ae04d70fce6d31cba6a57dde8) begin
            $display("FAIL enc_out mismatch got=%h exp=%h", enc_out, 136'h8895b7ed2ae04d70fce6d31cba6a57dde8);
            $finish;
        end

        $display("PASS sdmc_aead128_pt1");
        $finish;
    end
