...
module tb_sdmc_aead128_pt2;

    localparam TOKEN_W = `SDMC_TOKEN_W;
    localparam OUT_BYTES = 18;
    ...
    wire [TOKEN_W-1:0] token4 = {1'b1, `SDMC_TOK_MSG,   4'd2, 64'h0000000000002120};
    ...
    always @(posedge clk) begin
        if (!rst_n || clear) begin
            ...
            if (out_idx == 6'd0) begin
                if (tok_kind !== `SDMC_TOK_OUT || tok_bytes !== 4'd2) begin
    ...
    initial begin
        $dumpfile("tb_sdmc_aead128_pt2.vcd");
        $dumpvars(0, tb_sdmc_aead128_pt2);
        ...
        if (enc_out !== 144'h975146f61854eef62462e796a3d2125ac3e8) begin
            $display("FAIL enc_out mismatch got=%h exp=%h", enc_out, 144'h975146f61854eef62462e796a3d2125ac3e8);
            $finish;
        end

        $display("PASS sdmc_aead128_pt2");
        $finish;
    end
