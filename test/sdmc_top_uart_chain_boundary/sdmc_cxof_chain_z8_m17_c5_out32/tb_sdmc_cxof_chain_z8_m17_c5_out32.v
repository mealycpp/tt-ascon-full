...
module tb_sdmc_cxof_chain_z8_m17_c5_out32;
    ...
    initial begin
        ...
        wait_cycles(20);
        rst_n = 1'b1;
        // Auto-generated top-UART chain vector: sdmc_cxof_chain_z8_m17_c5_out32
        // Source core TB: test/sdmc_chain_vector_matrix/sdmc_cxof_chain_z8_m17_c5_out32/tb_sdmc_cxof_chain_z8_m17_c5_out32.v
        msg[0] = 8'h00;
        msg[1] = 8'h01;
        msg[2] = 8'h02;
        msg[3] = 8'h03;
        ...
        $display("DBG CHAIN_TOP start name=sdmc_cxof_chain_z8_m17_c5_out32 mode=%0d msg=%0d cs=%0d out=%0d",
        ...
        if (md_rx_count != OUT_LEN) begin
            $display("FAIL CHAIN_TOP_TIMEOUT name=sdmc_cxof_chain_z8_m17_c5_out32 rx=%0d expected=%0d",
                     md_rx_count, OUT_LEN);
            errors = errors + 1;
        end
        ...
        for (i = 0; i < OUT_LEN; i = i + 1) begin
            if (got_md[i] !== exp_md[i]) begin
                if (errors == 0) begin
                    $display("FAIL CHAIN_TOP_FIRST_MISMATCH name=sdmc_cxof_chain_z8_m17_c5_out32 idx=%0d got=%02x exp=%02x",
                             i, got_md[i], exp_md[i]);
                end
                errors = errors + 1;
            end
        ...
        $write("GOT_CHAIN_TOP name=sdmc_cxof_chain_z8_m17_c5_out32 got=");
        for (i = 0; i < OUT_LEN; i = i + 1) $write("%02x", got_md[i]);
        $display("");

        $write("EXP_CHAIN_TOP name=sdmc_cxof_chain_z8_m17_c5_out32 exp=");
        for (i = 0; i < OUT_LEN; i = i + 1) $write("%02x", exp_md[i]);
        $display("");

        if (errors == 0)
            $display("PASS CHAIN_TOP name=sdmc_cxof_chain_z8_m17_c5_out32 mode=%0d msg=%0d cs=%0d out=%0d",
                     MODE, MSG_LEN, CS_LEN, OUT_LEN);
        else
            $display("FAIL CHAIN_TOP name=sdmc_cxof_chain_z8_m17_c5_out32 errors=%0d", errors);
