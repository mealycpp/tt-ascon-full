...
module tb_hash_c0021_m20;
    ...
    initial begin
        ...
        $display("DBG HASH start name=hash_c0021_m20 Count=21 MSG_LEN=%0d t=%0t", MSG_LEN, $time);
        ...
        if (md_rx_count != MD_LEN) begin
            $display("FAIL HASH_TIMEOUT name=hash_c0021_m20 Count=21 MSG_LEN=%0d md_rx_count=%0d",
                     MSG_LEN, md_rx_count);
            errors = errors + 1;
        end
        ...
        for (i = 0; i < MD_LEN; i = i + 1) begin
            if (got_md[i] !== exp_md[i]) begin
                if (errors == 0) begin
                    $display("FAIL HASH_FIRST_MISMATCH name=hash_c0021_m20 Count=21 MSG_LEN=%0d idx=%0d got_byte=%02x exp_byte=%02x",
        ...
        $write("GOT_MD name=hash_c0021_m20 Count=21 MSG_LEN=%0d got=", MSG_LEN);
        for (i = 0; i < MD_LEN; i = i + 1) $write("%02x", got_md[i]);
        $display("");

        $write("EXP_MD name=hash_c0021_m20 Count=21 MSG_LEN=%0d exp=", MSG_LEN);
        for (i = 0; i < MD_LEN; i = i + 1) $write("%02x", exp_md[i]);
        $display("");

        if (errors == 0)
            $display("PASS HASH_KAT name=hash_c0021_m20 Count=21 MSG_LEN=%0d MD_LEN=32", MSG_LEN);
        else
            $display("FAIL HASH_KAT name=hash_c0021_m20 Count=21 MSG_LEN=%0d errors=%0d", MSG_LEN, errors);
