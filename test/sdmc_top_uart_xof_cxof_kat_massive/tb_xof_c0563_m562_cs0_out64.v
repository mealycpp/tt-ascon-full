...
module tb_xof_c0563_m562_cs0_out64;
    ...
    initial begin
        ...
        $display("DBG XOF_TOP start name=xof_c0563_m562_cs0_out64 mode=%0d msg=%0d cs=%0d out=%0d",
        ...
        if (md_rx_count != OUT_LEN) begin
            $display("FAIL XOF_TOP_TIMEOUT name=xof_c0563_m562_cs0_out64 rx=%0d expected=%0d",
                     md_rx_count, OUT_LEN);
            errors = errors + 1;
        end
        ...
        for (i = 0; i < OUT_LEN; i = i + 1) begin
            if (got_md[i] !== exp_md[i]) begin
                if (errors == 0) begin
                    $display("FAIL XOF_TOP_FIRST_MISMATCH name=xof_c0563_m562_cs0_out64 idx=%0d got=%02x exp=%02x",
                             i, got_md[i], exp_md[i]);
                end
                errors = errors + 1;
            end
        ...
        $write("GOT_XOF_TOP name=xof_c0563_m562_cs0_out64 got=");
        for (i = 0; i < OUT_LEN; i = i + 1) $write("%02x", got_md[i]);
        $display("");

        $write("EXP_XOF_TOP name=xof_c0563_m562_cs0_out64 exp=");
        for (i = 0; i < OUT_LEN; i = i + 1) $write("%02x", exp_md[i]);
        $display("");

        if (errors == 0)
            $display("PASS XOF_TOP name=xof_c0563_m562_cs0_out64 mode=%0d msg=%0d cs=%0d out=%0d",
                     MODE, MSG_LEN, CS_LEN, OUT_LEN);
        else
            $display("FAIL XOF_TOP name=xof_c0563_m562_cs0_out64 errors=%0d", errors);
