...
TESTDIR="test/cocotb_anubis_chain"
REPORTDIR="reports/anubis_cocotb_chain_core"
...
mapfile -t vectors < <(
  find test/sdmc_top_uart_chain_boundary -type f -name "tb_sdmc_*_chain_*.v" | sort
)

pass=0
fail=0

echo -e "${BOLD}=== ANUBIS-SDMC COCOTB XOF-CHAIN/CXOF-CHAIN TOP-UART TABLE ===${NC}"
printf "+----------------------------------------------+------------+------------+----------------------------------------------+\n"
...
if [ "$fail" -eq 0 ] && [ "$pass" -gt 0 ]; then
    ...
    echo -e "${GREEN}${BOLD}   \\(^_^)/  XOF-CHAIN/CXOF-CHAIN COCOTB PASSED  \\(^_^)/${NC}"
    echo -e "${GREEN}${BOLD}============================================================${NC}"
    exit 0
else
    echo
    echo -e "${RED}${BOLD}============================================================${NC}"
    echo -e "${RED}${BOLD}           (T_T)  CHAIN COCOTB FAILURES  (T_T)${NC}"
    echo -e "${RED}${BOLD}============================================================${NC}"
    exit 1
fi
