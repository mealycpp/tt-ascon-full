...
mkdir -p reports
RESULTS="reports/sdmc_top_uart_aead_matrix_results.tsv"
LOGDIR="reports/top_uart_aead_matrix_logs"
mkdir -p "$LOGDIR"
: > "$RESULTS"

python3 tools/sdmc_generate_top_uart_aead_matrix.py
...
while read -r name; do
  ...
  tb="test/sdmc_top_uart_aead_matrix/${name}/tb_${name}.v"
  vvp="test/sdmc_top_uart_aead_matrix/${name}/tb_${name}.vvp"
  log="${LOGDIR}/${name}.log"
  ...
  if ! iverilog -g2012 -I src -I src/sdmc \
    ...
    echo -e "${RED}FAIL ❌ ${name} [COMPILE] ${detail}${RESET}"
    FAIL_COUNT=$((FAIL_COUNT + 1))
    continue
  fi

  if timeout 60s vvp "$vvp" >> "$log" 2>&1; then
    if grep -q "PASS ${name}" "$log"; then
      ...
      printf "%s\tFAIL\tSIM\t%s\n" "$name" "$detail" >> "$RESULTS"
      echo -e "${RED}FAIL ❌ ${name} [SIM] ${detail}${RESET}"
      FAIL_COUNT=$((FAIL_COUNT + 1))
    fi
  else
    detail="$(grep -E "FAIL|ERROR|timeout" "$log" | tail -n 1 | sed 's/\t/ /g')"
...
done < test/sdmc_top_uart_aead_matrix/manifest.txt

echo
echo "=== TOP UART AEAD MATRIX SUMMARY ==="
printf "+------------------------------------------------------+--------+---------+------------------------------------------------------------+\n"
...
if [ "$FAIL_COUNT" -eq 0 ]; then
  echo -e "${GREEN}PASS sdmc_top_uart_aead_matrix_table${RESET}"
  exit 0
else
  echo -e "${RED}FAIL sdmc_top_uart_aead_matrix_table${RESET}"
